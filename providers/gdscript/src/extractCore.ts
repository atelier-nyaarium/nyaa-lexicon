import type { ImportedName, Literal, Metrics, Reference } from "@nyaa-lexicon/protocol";
...
function maskLine(text: string, state: StringState): string {
	const cursor = new Cursor(text);
	let code = "";
	while (cursor.good()) {
		const before = cursor.offset;
		if (state.triple !== null) {
			if (tripleQuoteAt(cursor, state.triple)) {
				code += consumeMasked(cursor, 3);
				state.triple = null;
			} else if (cursor.peek() === "\\") {
				code += masked(cursor.next());
				if (cursor.good()) code += masked(cursor.next());
			} else {
				code += masked(cursor.next());
			}
		} else if (cursor.peek() === "#") {
			...
			if (tripleQuoteAt(cursor, quote)) {
				code += consumeMasked(cursor, 3);
				state.triple = quote;
			} else {
				code += masked(cursor.next());
				while (cursor.good()) {
					const stringCharacter = cursor.next();
					code += masked(stringCharacter);
					if (stringCharacter === "\\" && cursor.good()) code += masked(cursor.next());
...
function readLines(text: string): SourceLine[] {
	...
	const state: StringState = { triple: null };
	let line = cursor.readLine();
	while (line !== null) {
		lines.push({ ...line, code: maskLine(line.text, state) });
...
function extractGdscript(module: string, text: string, compose: ComposeSymbolId): DeclarationFact[] {
	...
	const rootLine = classLine?.line ?? { line: 0, text: "", code: "" };
